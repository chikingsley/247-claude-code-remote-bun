 ...
 */
function ensureStopScript(): void {
  if (!existsSync(STATUSLINE_DIR)) {
    mkdirSync(STATUSLINE_DIR, { recursive: true });
  }
...
function ensureClaudeSettings(): void {
  ...
  const stopHooks = currentHooks?.Stop as
    | Array<{ hooks?: Array<{ command?: string }> }>
    | undefined;
  const stopConfigured = stopHooks?.some((h) =>
    h.hooks?.some((hook) => hook.command === expectedStopCommand)
  );
  ...
  if (!stopConfigured) {
    ...
    const existingStopHooks = (hooks.Stop || []) as Array<{
      hooks?: Array<{ command?: string }>;
    }>;

    // Filter out any old 247 stop hooks that might exist
    const userStopHooks = existingStopHooks.filter(
      (h) => !h.hooks?.some((hook) => hook.command?.includes('.247/stop.sh'))
    );
