...
describe("SessionModule", () => {
  ...
  describe("environment badge", () => {
    it("shows environment badge when session has environment", () => {
      const hasEnvironment = (session: { environment?: any }) =>
        !!session.environment;
  ...
  describe("real-time indicator", () => {
    it("shows zap icon for hook status source", () => {
      const showRealtimeIndicator = (statusSource?: string) =>
        statusSource === "hook";
  ...
  describe("git worktree features", () => {
    it("detects worktree sessions", () => {
      const hasWorktree = (session: { worktreePath?: string }) =>
        !!session.worktreePath;
      expect(
        hasWorktree({ worktreePath: "/tmp/247-workspaces/session-1" })
      ).toBe(true);
    ...
    it("shows branch name when available", () => {
      const getBranchName = (session: { branchName?: string }) =>
        session.branchName;
      expect(getBranchName({ branchName: "session/feature-x" })).toBe(
        "session/feature-x"
      );
    ...
    it("git buttons only visible for worktree sessions", () => {
      const showGitButtons = (session: { worktreePath?: string }) =>
        !!session.worktreePath;
