 ...
 */
import { buildApiUrl, buildWebSocketUrl, cn, stripProtocol } from "@/lib/utils";
...
describe("Utils", () => {
  ...
  describe("stripProtocol", () => {
    ...
    it("preserves port numbers", () => {
      expect(stripProtocol("https://example.com:4678")).toBe(
        "example.com:4678"
      );
    });
  ...
  describe("buildWebSocketUrl", () => {
    it("builds ws:// URL for localhost", () => {
      expect(buildWebSocketUrl("localhost:4678", "/status")).toBe(
        "ws://localhost:4678/status"
      );
    });

    it("builds ws:// URL for 127.0.0.1", () => {
      expect(buildWebSocketUrl("127.0.0.1:4678", "/status")).toBe(
        "ws://127.0.0.1:4678/status"
      );
    });
    ...
    it("handles Tailscale URLs with protocol", () => {
      expect(
        buildWebSocketUrl(
          "https://macbook-pro.tail5f910b.ts.net:4678",
          "/status"
        )
      ).toBe("wss://macbook-pro.tail5f910b.ts.net:4678/status");
    });

    it("handles Tailscale URLs without protocol", () => {
      expect(
        buildWebSocketUrl("macbook-pro.tail5f910b.ts.net:4678", "/status")
      ).toBe("wss://macbook-pro.tail5f910b.ts.net:4678/status");
    });

    it("handles paths with query parameters", () => {
      expect(
        buildWebSocketUrl("localhost:4678", "/terminal?project=test&session=s1")
      ).toBe("ws://localhost:4678/terminal?project=test&session=s1");
    });

    it("handles complex paths", () => {
      expect(
        buildWebSocketUrl("https://example.com:4678", "/api/v1/websocket")
      ).toBe("wss://example.com:4678/api/v1/websocket");
    });
  });
  ...
  describe("buildApiUrl", () => {
    ...
    it("handles Tailscale URLs with protocol", () => {
      ...
      ).toBe("https://macbook-pro.tail5f910b.ts.net:4678/api/sessions");
    });

    it("handles Tailscale URLs without protocol", () => {
      expect(
        buildApiUrl("macbook-pro.tail5f910b.ts.net:4678", "/api/sessions")
      ).toBe("https://macbook-pro.tail5f910b.ts.net:4678/api/sessions");
    });
    ...
    it("handles complex paths", () => {
      expect(
        buildApiUrl("https://example.com:4678", "/api/sessions/test/archive")
      ).toBe("https://example.com:4678/api/sessions/test/archive");
    });
    ...
    it("handles URL with double protocol correctly", () => {
      ...
      expect(
        ...
        "https://macbook-pro.tail5f910b.ts.net:4678/api/sessions/archived"
      );
    });
  });
});
