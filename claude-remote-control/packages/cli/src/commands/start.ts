...
import { configExists, getProfilePath, loadConfig } from "../lib/config.js";
import { getAgentPaths } from "../lib/paths.js";
import { isAgentRunning, startAgentDaemon } from "../lib/process.js";
...
export const startCommand = new Command("start")
  ...
  .action(async (options, cmd) => {
    ...
    if (status.running) {
      console.log(
        chalk.yellow(`Agent is already running (PID: ${status.pid})\n`)
      );
    ...
    if (options.foreground) {
      ...
      console.log(
        chalk.blue(
          `Starting agent${profileLabel} in foreground on port ${config.agent.port}...\n`
        )
      );
      ...
      if (result.success) {
        ...
        console.log();
        console.log(
          `Agent running on ${chalk.cyan(`http://localhost:${config.agent.port}`)}`
        );
        console.log();
      } else {
