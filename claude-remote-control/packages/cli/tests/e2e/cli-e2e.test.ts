...
if (skipE2E) {
  ...
  if (!hasTmux) {
    console.warn("  - tmux not installed");
  }
  if (!hasNode22) {
    console.warn("  - Node.js 22+ required");
  }
}
...
describe.skipIf(skipE2E)("247 CLI E2E Tests", () => {
  ...
  describe("init command", () => {
    ...
    it("refuses to overwrite without --force", async () => {
      ...
      const config = env.readJson<{ machine: { name: string } }>(
        ".247/config.json"
      );
  ...
  describe("service command (platform-specific)", () => {
    it.skipIf(platform() !== "darwin")(
      "creates launchd plist on macOS",
      async () => {
        const port = await getFreePort();
        await env.runCli([
          "init",
        ...
        expect(env.fileExists("Library/LaunchAgents/com.quivr.247.plist")).toBe(
          true
        );
    ...
    it.skipIf(platform() !== "linux")(
      ...
      async () => {
        ...
        expect(env.fileExists(".config/systemd/user/247-agent.service")).toBe(
          true
        );
  ...
  describe("start/stop lifecycle", () => {
    ...
    it("starts and creates PID file", async () => {
      ...
      const startResult = await env.runCli(["start"], { timeout: 15_000 });
      ...
      if (startResult.exitCode === 0) {
        ...
        const pid = Number.parseInt(env.readFile(".247/agent.pid"), 10);
