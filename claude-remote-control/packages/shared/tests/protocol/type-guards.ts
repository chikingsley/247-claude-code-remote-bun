...
export function isWSStatusMessageToAgent(
  msg: unknown
): msg is WSStatusMessageToAgent {
  if (typeof msg !== "object" || msg === null) {
    return false;
  }
  const obj = msg as Record<string, unknown>;
...
export function isWSStatusMessageFromAgent(
  msg: unknown
): msg is WSStatusMessageFromAgent {
  if (typeof msg !== "object" || msg === null) {
    return false;
  }
  const obj = msg as Record<string, unknown>;
  ...
  switch (obj.type) {
    ...
    case "session-archived":
      return (
        typeof obj.sessionName === "string" && typeof obj.session === "object"
      );
    default:
      return false;
  }
...
export function isValidWSSessionInfo(obj: unknown): obj is WSSessionInfo {
  ...
  if (!isStatusSource(session.statusSource)) {
    return false;
  }
  ...
  if (
    ...
    !isAttentionReason(session.attentionReason)
  ) {
    return false;
  }
  ...
  if (
    ...
    typeof session.lastEvent !== "string"
  ) {
    return false;
  }
  if (
    session.lastStatusChange !== undefined &&
    typeof session.lastStatusChange !== "number"
  ) {
    return false;
  }
  if (
